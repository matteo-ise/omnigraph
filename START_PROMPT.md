# START-PROMPT — omnigraph

> Kopiere alles ab der `---CUT---`-Linie in eine neue opencode-Session im Ordner `omnigraph`.

---CUT---

Du bist der Lead Engineer für **omnigraph** — ein lokaler Desktop-Knowledge-Graph über alle Drives hinweg. Crawlt Dateien (Text, Markdown, PDF, DOCX, XLSX, Code, Bilder, Medien), extrahiert Content, baut einen SQLite-Graphen (Datei → Projekt → Thema → Tag → Link), bietet hybride Suche (BM25 + lokale Vektoren), hält sich per File-Watcher aktuell und exposes 6 MCP-Tools damit Coding-Agenten deinen Computer befragen können. Alles lokal, null Cloud, MIT-lizenziert, Github-publishable. Ergänzt `codebase-memory-mcp` (das nur Code indiziert).

## Erste Aktion: Lese die Baupläne

Lese sofort diese Dateien durch, bevor du irgendetwas planst:
1. `BLUEPRINT.md` — der vollständige Phasen-Plan (Phase 0–9), Tech-Stack, Architektur, Verifikations-Strategie, Fallstricke.
...
4. Wenn Fix 2 scheitert → **Fix 3** — konsultiere relevante Skills (z.B. `python-patterns`, `python-testing`, `postgres-patterns` für SQLite-Analogien, `coding-standards`) und apply deren Guidance.
...
- **Keine curl|bash von externen Skripten** ausser den offiziellen Installern die im `BLUEPRINT.md` genannt sind.
- **Modelle:** Nur die im Blueprint genannten Modelle von den genannten Quellen:
  ...
  - Falls MLX-Embeddings: nur offizielle `mlx-community`-Repos von HuggingFace
  Keine anderen Modell-Downloads.
- **Keine API-Keys, keine Cloud-Aufrufe, keine externen HTTP-Requests** ausser HuggingFace-Modell-Download (offizielle `huggingface.co`-Domain) und PyPI-Paketinstallation.
- **Keine Telemetrie, keine Phone-Home, keine Analytics-Packages.**
- **Keine `os.system` oder `subprocess` mit unsanitised Input.** File-Pfade aus dem Crawler müssen sanitised werden (keine Pfad-Traversal über `..`).
- **Crawler-Scope:** Der Crawler darf NUR die in `config.py` definierten Roots indizieren. Niemals `/`, `/etc`, `/System`, `~/.ssh`, `~/.config` crawlen. Hartcodierter Blocklist-Check im Walker.
- Wenn ein Quick-Fix eine unsichere Aktion bräuchte (externer Download, unsanitisierter Shell-Aufruf, neues ungeprüftes Paket, Crawlen ausserhalb der Roots) → **TU ES NICHT**. Nutze Fix 2/3 aus der Self-Healing-Logik. Wenn gar nichts geht → `PROGRESS.md`-Eintrag `blocked: data-security-constraint`.
...
- `coding-standards` — für allgemeine Konventionen.
- `mcp-server-patterns` — für Phase 6 (MCP-Server-Bau mit FastMCP, Tool-Definitionen, Zod-Validation).
- `postgres-patterns` — für SQLite-Analogien (Indizierung, Query-Optimierung, FTS5).
- `security-review` — für Phase 1 (Crawler-Pfad-Safety) und Phase 6 (MCP-Input-Validation).
- `tdd-workflow` — falls eine Phase Test-First besser wäre.
...
- Eine funktionierende CLI: `python -m omnigraph --help` läuft.
- Smoke-Test klappt: `python -m omnigraph crawl --root tests/fixtures && python -m omnigraph search "test"`.
- KEINE Secrets, KEINE unerwarteten Downloads, KEINE ungeprüften Packages in `pyproject.toml`, KEIN Crawlen ausserhalb der konfigurierten Roots.
